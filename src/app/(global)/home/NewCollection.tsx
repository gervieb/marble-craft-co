...
const NewCollection = () => {
    ...
    return (
        <Box>
            <Typography sx={{ fontSize: 18, textAlign: 'center', }}>NEWEST COLLECTION</Typography>

            <Stack direction='row' display='flex' justifyContent='space-around' alignItems='center' spacing={4} mt={4}>
                <Box sx={{ width: '750px' }}>
                    <Image src={newProducts[0].url} alt='christmas marble product' width="0"
                        height="0"
                        sizes="100vw"
                ...
                <Grid container spacing={2} mt={4} alignItems='center'>
                    {removeFirstProduct.map((product, idx) => (
